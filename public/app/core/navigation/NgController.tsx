...
class NgController extends React.Component<NgControllerProps, {}> {
  ...
  componentDidUpdate(prevProps: NgControllerProps) {
    const { location: { search }, reloadOnSearch, injector, mountContainer } = this.props;
  ...
  async mountController() {
    ...
    routeLocals['$scope'] = this.childScope;
    $route.updateRouteLocals(routeLocals);

    // TODO: not sure if this is the place I want do emit this event
    ...
    if (routeDescriptor.controller) {
      ...
      const ctrl = $controller(routeDescriptor.controller, routeLocals);
    ...
    $compile(mountContainer.querySelector('#ngRoot'))(this.childScope);
    this.setBodyClass();
    this.mounted = true;
  }
